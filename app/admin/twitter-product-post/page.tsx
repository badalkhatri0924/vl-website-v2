...
import { Copy, Check, ExternalLink, ChevronDown, ArrowLeft, Download, Eye, Trash2 } from 'lucide-react'
...
export default function TwitterProductPostPage() {
  const [userName, setUserName] = useState('')
  const [batches, setBatches] = useState<TwitterPostBatch[]>([])
  const [loadingBatches, setLoadingBatches] = useState(false)
  const [showTwitterForm, setShowTwitterForm] = useState(false)
  const [productName, setProductName] = useState('')
  const [productUrl, setProductUrl] = useState('')
  const [generating, setGenerating] = useState(false)
  const [twitterPosts, setTwitterPosts] = useState<TwitterPostItem[]>([])
  ...
  const fetchBatches = async () => {
    ...
    try {
      const res = await fetch('/api/twitter/list')
  ...
  useEffect(() => {
    ...
    const productMap: Record<string, TwitterPostBatch> = {}
    batches.forEach((b) => {
  ...
  const handleGenerateTwitter = async (e: React.FormEvent) => {
    e.preventDefault()
    ...
    try {
      ...
      if (res.ok && data.success && Array.isArray(data.posts)) {
        const saveRes = await fetch('/api/twitter/save', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            productName: productName.trim(),
  ...
  const copyPost = async (
    ...
    if (options) {
      setClaimingKey(key)
      try {
        const res = await fetch('/api/twitter/copy', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
    ...
    return (
      ...
      >
        ...
        <CardContent className="relative p-4">
          ...
          {post.hook && (
            <p className="text-slate-200 font-bold text-lg mb-3">{post.hook}</p>
          )}
  ...
  return (
    <div className="min-h-screen bg-obsidian-950 text-white pt-28 md:pt-32 pb-24 px-4 md:px-8">
      <div className="w-full max-w-full px-4 md:px-32">
        ...
        <h1 className="text-3xl md:text-4xl font-display font-black mb-2">Twitter product post</h1>
        <p className="text-slate-400 mb-8">Generate and manage product-based Twitter (X) content. Problem/solution style, 280 chars.</p>
        ...
        <h2 className="text-xl font-display font-bold mb-4">Saved Twitter content</h2>
        {loadingBatches ? (
          <div className="flex flex-col items-center justify-center min-h-[160px] gap-3 text-center">
            ...
            <p className="text-xs text-slate-400">Loading saved Twitter content…</p>
          </div>
        ) : batches.length === 0 ? (
      ...
      <Dialog open={showTwitterForm} onOpenChange={setShowTwitterForm}>
        <DialogContent className="w-full max-w-full md:max-w-xl max-h-[90vh] overflow-y-auto" onClose={() => setShowTwitterForm(false)}>
          <DialogHeader>
            <DialogTitle>Generate Twitter content</DialogTitle>
          </DialogHeader>
          <form onSubmit={handleGenerateTwitter} className="space-y-6">
            <div>
          ...
          {generating && (
            <div className="mt-6 flex flex-col items-center gap-3 text-center">
              <Avatar state="thinking" className="w-20 h-20" />
              <p className="text-xs text-slate-400">We’re analyzing the page and drafting your Twitter posts…</p>
            </div>
          )}
