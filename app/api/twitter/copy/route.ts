...
import { claimTwitterPostCopy } from '@/lib/twitterPosts'
...
export async function POST(request: NextRequest) {
  try {
    ...
    const claimed = await claimTwitterPostCopy(batchId, postIndex, copiedBy)
    if (!claimed) {
      return NextResponse.json(
    ...
    console.error('Error claiming Twitter copy:', err)
    return NextResponse.json(
